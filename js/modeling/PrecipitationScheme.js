        ...
        PrecipitationScheme.prototype.calcPrecipCouche = function(k)
        {
            ...
            for (var i=0;i<this.model.Pl[k].length;i++)
            {
                ...
                if (this.model.qv[k][i]>qsat)
                {
                    ...
                    this.model.P_evap[k][i] = this.model.qv[k][i]-qsat;
                    this.model.Pl[k+1][i] = this.model.Pl[k][i] + (this.model.P_evap[k][i])*(this.model.p[k_tilde][i]-this.model.p[k_tilde1][i])/(this.model.dt*Model.g);
                }
                else
                {
                    // Evaporation....
                    ...
                    {
                        this.model.P_evap[k][i] = this.model.qv[k][i]-qsat;
                        P_temp = Math.sqrt(this.model.Pl[k][i]) + 4.8e6*(this.model.P_evap[k][i])*(1/this.model.p[k_tilde][i]-1/this.model.p[k_tilde1][i]);
